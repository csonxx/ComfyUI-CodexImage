...
`CODEX_IMAGE_OPENROUTER_MODEL`、`CODEX_IMAGE_OPENROUTER_GEMINI_MODEL`、`CODEX_IMAGE_LITELLM_MODEL`、`CODEX_IMAGE_REQUESTY_MODEL` 和 `CODEX_IMAGE_WAVESPEED_MODEL` 只是默认值，节点上仍然可以手动填写或选择模型名。
...
`OpenRouter Image (GPT Image 2)`、`OpenRouter Gemini Image` 和 `LiteLLM Image (GPT Image 2)` 也支持可选 `image` / `image_2` / `mask`。不接图片时就是纯 prompt 生图。`OpenRouter Gemini Image` 使用 Gemini/Nano Banana 的 `resolution` 和 `aspect_ratio` 参数，不会发送 GPT Image 2 的 `size` / `quality` / `background` 字段；mask 会烘到第一张参考图的 alpha 通道里，按参考图引导处理。
