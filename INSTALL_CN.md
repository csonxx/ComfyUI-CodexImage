...
`Requesty Response I2I (GPT Image 2)` 只做 I2I，主图必填。它是更接近 Codex 效果的 Requesty 节点：图片组织方式对齐 `Codex Image I2I (GPT Image 2)`，请求走 Requesty `/v1/responses`，payload 使用原生 `tools: [{"type": "image_generation", "action": "edit", ...}]`。默认模型是 `openai-responses/gpt-5.5`；不要改成普通 `openai/` 前缀，否则 Requesty 可能转到 Chat Completions。
