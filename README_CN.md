...
| `Requesty Response I2I (GPT Image 2)` | Codex 风格 I2I 图片组织方式，再发 Requesty `/v1/responses` 原生 `image_generation` + `action=edit` | 可选节点 `api_key`，否则读 `REQUESTY_API_KEY` |
...
`Requesty Response I2I (GPT Image 2)` 是 Requesty 里最接近 `Codex Image I2I (GPT Image 2)` 的路径。它同样使用 Codex I2I 图片组织方式，然后向 `https://router.requesty.ai/v1/responses` 发送原生 Responses payload：`tools: [{"type": "image_generation", "action": "edit", ...}]`。默认 `model` 是 `openai-responses/gpt-5.5`；要保留 `openai-responses/` 前缀，这样 Requesty 才会走原生 Responses API，而不是把 `openai/` 前缀转到 Chat Completions。
...
| `CODEX_IMAGE_REQUESTY_RESPONSES_MODEL` | `openai-responses/gpt-5.5` | Requesty Responses I2I 节点默认模型 |
