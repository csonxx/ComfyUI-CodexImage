...
| `OpenRouter Gemini Image` | OpenRouter dedicated Images API，使用 Gemini/Nano Banana 的 `resolution` + `aspect_ratio` 参数 | `CODEX_IMAGE_OPENROUTER_API_KEY` 或 `OPENROUTER_API_KEY` |
...
`OpenRouter Gemini Image` 是专门给 Gemini/Nano Banana 系列用的 OpenRouter Images API 节点。它发送 `resolution` 和 `aspect_ratio`，不会发送 GPT Image 2 的 `size`、`quality`、`background` 字段。节点内置 `google/gemini-2.5-flash-image`、`google/gemini-3.1-flash-image`、`google/gemini-3-pro-image` 等 OpenRouter Gemini image model，并会在请求前校验已知的 resolution/aspect-ratio 支持范围。可选 `image`、`image_2` 和 `mask` 会作为 `input_references` 发出；mask 会烘到第一张图的 alpha 通道里，这更接近参考图引导，不等于 OpenAI-style 强 mask edit。
...
| `CODEX_IMAGE_OPENROUTER_GEMINI_MODEL` | `google/gemini-2.5-flash-image` | OpenRouter Gemini 节点默认模型 |
